...
export default function SignupPage() {
  ...
  const { user, signInWithGoogle, loading: authLoading, signUpWithEmailPassword } = useAuth();
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      alert("Passwords don't match!"); // Replace with better error handling (e.g., toast)
      return;
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      console.error("Google Sign-In failed (from signup page):", error);
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md shadow-xl">
        <CardHeader className="text-center">
          ...
          <CardDescription>Join Global News AI today</CardDescription>
        </CardHeader>
        <CardContent>
